...
def collision(ball_1, ball_2):
    ...
    ball_1.v = [float('%.8f'%(vx1 - dot1 * (x1 - x2))), float('%.8f'%(vy1 - dot1 * (y1 - y2)))]
    ball_2.v = [float('%.8f'%(vx2 - dot2 * (x2 - x1))), float('%.8f'%(vy2 - dot2 * (y2 - y1)))]
...
class Ball:
    ...
    def __init__(
            ...
            velocity_y):
        self.id = ball_id
        self.p = [position_x, position_y]
        self.v = [velocity_x, velocity_y]
    ...
    for j in times:
        ...
        if collisionTimes != -1:
            ...
            while True:
                ...
                for i in collisionTimes[k]:
                    collision(i[0], i[1])  # Do collide
